    ...
    <AppLayout title="My Offers">
        ...
        <div class="py-8">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <!-- Offers Table -->
                <div v-if="offers.data.length > 0" class="overflow-x-auto bg-white rounded-xl shadow-sm">
                    <table class="min-w-full divide-y divide-gray-200">
                        <thead class="bg-gray-50">
                            <tr>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Offer</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Model</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Clicks</th>
                                <th class="px-4 py-3 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider">Conversions</th>
                        ...
                        <tbody class="divide-y divide-gray-200 bg-white">
                            <tr v-for="offer in offers.data" :key="offer.id" class="hover:bg-gray-50">
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            ...
                                            <div class="text-xs text-gray-500 truncate max-w-xs">{{ offer.description }}</div>
                                        </div>
                                    </div>
                                </td>
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ offer.total_clicks }}</td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ offer.total_conversions }}</td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ formatShortCurrency(offer.total_revenue) }}</td>
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <span :class="[
                                            'inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold',
                                            offer.approval_status === 'pending' ? 'bg-yellow-100 text-yellow-800' : offer.approval_status === 'rejected' ? 'bg-red-100 text-red-800' : offer.is_active ? 'bg-green-100 text-green-800' : 'bg-gray-100 text-gray-700'
                                        ]">
                                        {{ offer.approval_status === 'pending' ? 'Pending' : offer.approval_status === 'rejected' ? 'Rejected' : offer.is_active ? 'Active' : 'Inactive' }}
                                    </span>
                                </td>
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900">{{ offer.access_type === 'open' ? 'Open' : 'Request' }}</td>
